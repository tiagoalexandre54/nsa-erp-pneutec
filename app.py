...
from modules.itinerario import tela_itinerario
...
menu = st.sidebar.radio(
    "Selecione a Tela:",
    ["📊 Painel PPCP", "📥 Recebimento", "🏗️ Pneus a Produzir", "🏭 Entrada (Chão de Fábrica)", "🚚 Expedição (Carga)", "🗺️ Itinerário", "📈 Relatórios"]
)
...
if menu == "📊 Painel PPCP":
    ...
    tela_recebimento()
elif menu == "🏗️ Pneus a Produzir":
    tela_producao_diaria()
elif menu == "🏭 Entrada (Chão de Fábrica)":
    tela_entrada()
elif menu == "🚚 Expedição (Carga)":
    tela_expedicao()
elif menu == "🗺️ Itinerário":
    tela_itinerario()
